...
| 4 | **Read** | Extract values and visible state from discovered controls. | Can the system read enough state to support agent reasoning and verification? | `sgcl read --window <wid> --target <id>` returns value, state, selection, visible text. | Read works for text fields, checkboxes, radios, list selection, status text. | Find. | Some controls expose no readable value via accessibility. |
...
| 11 | **Agent Integration Surface (MCP)** | Expose the command vocabulary as MCP tools over a warm process instead of subprocess-and-parse. | Is per-invocation cost a real problem or an assumed one — and does tool-call ergonomics change what an agent can do? | A stateless MCP server wrapping the existing `Adapter`; the same verbs, the same JSON. | An agent completes the Win 10 task through MCP tool calls, and the per-command latency difference against the CLI is measured and recorded. | Agent Loop. | Building it before Phase 3 settles snapshot caching, then retrofitting state. |
...
Win 11 is deliberately **after** Win 10 rather than before it. The daemon was a stated goal in the legacy spec that this roadmap dropped without deciding to (see [`ADR-0006`](decisions/ADR-0006-agent-integration-surface.md)), so it is listed here to stop it being invisible. It is gated on Win 10 because Win 10 is what produces the number: nothing has yet measured whether per-invocation tree walks actually hurt an agent, and building a server on an assumed cost is how you get a daemon nobody needed.
